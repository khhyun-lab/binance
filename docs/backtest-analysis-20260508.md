...
## breakout post-filter 수익 probe

pullback 병목을 더 분해한 결과, 2026-04 pullback_balanced 기준 후보 743건의 핵심 차단 사유는 다음이었다.

- score_below_threshold: 444
- exhausted_entry: 214
- momentum_blocked: 51
- sideways_blocked: 30
- pullback_min_rr_blocked: 4

실제 체결 6건을 분해하면 다음 특징이 보였다.

- breakout_chase_short 2건은 모두 breakout_failure 손실
- breakout_chase_long 4건 중 1건만 take_profit, 3건은 breakout_failure 손실
- 유일한 수익 거래는 중간 수준 3분 RSI long breakout이었다.

이 관찰을 바탕으로 별도 profit probe를 수행했다.

| probe | 리포트 경로 | 거래 수 | 순손익 | 비고 |
| --- | --- | ---: | ---: | --- |
| breakout post-filter | reports/backtests_202604_profit_probe_v3/20260508T153100Z | 1 | 20.2379 | breakout short 기본 비활성화, 과열 long breakout veto |

이 probe는 거래 수를 크게 줄였지만, 2026-04 단일 월 기준으로는 손실 전략을 플러스로 뒤집었다. 따라서 현 시점의 수익 방향은 문턱 완화가 아니라 다음 두 가지에 가깝다.

1. breakout short는 기본 비활성화하고, 숏은 별도 edge가 확인된 구조에서만 다시 허용
2. long breakout은 허용하되 3분 RSI 과열과 ATR 대비 과도한 확장 구간은 주문 직전 veto
...
- 단, 2026-04 단일 월 profit probe 기준으로는 breakout short 제거 + 과열 long veto 조합이 순손익 플러스로 전환됐다.
- 새 pullback 재가속 구조는 아직 체결 기여를 만들지 못했고, 결과적으로 baseline과 동일한 거래/손익을 기록했다.
- 채택한 방향:
  - 멀티타임프레임 정렬 기반 진입 강화
  - 추격 진입 억제
  - 과열 롱 진입 억제
  - breakout short 기본 비활성화
...
1. breakout short를 다시 켤 근거가 생길 때까지는 별도 숏 구조를 분리해 검증
2. pullback 후보 743건 중 실제 체결 0건이 된 병목을 RR, reaccel, volume 조건 중 어디서 더 많이 잃는지 추가 분해
3. profit probe가 1건 체결에 그친 이유를 기준으로 long breakout 과열 veto를 다월 데이터에서 교차검증
